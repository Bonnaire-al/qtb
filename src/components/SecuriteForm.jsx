...
export default function SecuriteForm1({ onClose, onCancel }) {
  ...
  const handleSelectAll = () => {
    const allServices = securityServices.map(service => service.value);
  ...
  const handleAddToDevis = () => {
    if (selectedServices.length > 0) {
      const selectedServiceLabels = selectedServices.map(serviceValue => 
        securityServices.find(s => s.value === serviceValue)?.label
      ).filter(Boolean);
  ...
  const handleGenerateDevis = () => {
    console.log('Devis sécurité généré:', devisItems);
      ...
      <div className="max-h-[80vh] overflow-y-auto max-w-md mx-auto px-8">
        <form className="space-y-4">
          ...
          <div className="text-center mb-4">
            <h2 className="text-xl font-bold text-cyan-800">Système de sécurité</h2>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden">
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-cyan-800">Devis - Prestations de sécurité</h2>
              <button
